...
dataTrain = pd.read_csv("./xiguadata.csv", sep="\\t")

naiveBayes = NaiveBayes()
treeData = naiveBayes.fit(dataTrain)

print(json.dumps(treeData, indent=4, ensure_ascii=False))

pd = pd.DataFrame({'预测值': naiveBayes.predict(dataTrain), '正取值': dataTrain.iloc[:, -1]})
print('accurcye:%f%%' % (pd[pd['预测值'] == pd['正取值']].shape[0] * 100.0 / pd.shape[0]))
