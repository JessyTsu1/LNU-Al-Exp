...
dataset = pd.read_csv('watermelon_3.txt', delimiter="   ", engine='python')

dataset = np.array(dataset)
m, n = np.shape(dataset)
for i in range(m):
    for j in range(n):
        dataset[i, j] = round(dataset[i, j], 3)  # 定义数据格式，dataset[i,j]保留小数点后3位

trueY = dataset[:, n - 1]  # 取类别标签
X = dataset[:, :n - 1]  # 取属性部分
m, n = np.shape(X)  # 行列值
